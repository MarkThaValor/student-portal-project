  ...
  <div class="min-h-[80vh] flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-slate-50">
    <div class="w-full max-w-sm space-y-6">
      <div class="text-center space-y-2">
        ...
        <h2 class="text-xl font-bold text-slate-900 tracking-tight">Create your account</h2>
        <p class="text-xs text-slate-400 font-medium">Fill in the fields below to register a new student</p>
      </div>

      <div class="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm">
        <form class="space-y-4" @submit.prevent="handleRegister">
          <div class="space-y-1">
            <label for="username" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Username</label>
            <input 
              id="username" 
              type="text" 
              required 
          ...
          >
            ...
            {{ authLoading ? 'Signing up...' : 'Register' }}
          </button>
        </form>

        <div class="mt-5 text-center text-xs">
          <span class="text-slate-400 font-medium">Already registered? </span>
          <NuxtLink to="/login" class="font-bold text-indigo-600 hover:text-indigo-700 hover:underline">
            Login here
          </NuxtLink>
        </div>
      </div>
...
const handleRegister = async () => {
  ...
  if (!username.value || !email.value || !password.value) {
    error.value = 'Please fill in all registration fields.'
    return
  }
  ...
  try {
    ...
    error.value = 'Registration failed. Please try a different email.'
  }
}
</script>
